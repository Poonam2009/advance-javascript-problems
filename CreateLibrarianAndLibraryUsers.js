...
class Library {
  ...
  /**
   * Create new methods:
   * - addUser(user) - adds user to the users array
   * - findUser(name) - returns user object
   * - setLibrarian(name) - sets only one librarian
   */
...
class LibraryUser {
   ...
   * - - Returns `Book is available: ${book.getDetails()}`
   * - - Returns `"${title}" is currently on loan.`
   * - - Returns `Book titled "${title}" not found.`
   * 
   * - loanBook(library, title):
   * - - If user is suspended, return `${this.name} is suspended and cannot borrow books.`
   * - - If book is available, add to user borrowedBooks array, return `${this.name} has borrowed the book "${title}" from ${librarian.name}.`
   * - - else, return `"${title}" is currently on loan.`
   * - - else, return `Book titled "${title}" not found.`
   */
  // Your code begins here.
...
class Librarian extends LibraryUser {
   ...
   * - - If user has book, remove book from borrowedBooks array, set book status to "available", return `${this.name} has checked in the book "${title}" from ${userName}.`
   * - - else, return `"${title}" not found in user's borrowed books.`
   * - - else, return "Book or user not found."
   * 
   * - suspendUser(library, userName)
   * - - If user exists, set suspended prop to true, return `${this.name} has suspended ${userName}.`
